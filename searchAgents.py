...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            x,y = state[0]
            visited = state[1]
            dx, dy = Action.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    from util import searchDistance

    return 0 # Default to trivial solution

    pos = state [0]
    remained_corner = [corner for corner in corners if corners not in state[1]]

    while len(remained_corner) > 0:
        distance_array = [searchDistance(now, corner) for corner in remained_corner]
